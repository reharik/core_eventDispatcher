...
var eventDispatcher = function eventDispatcher(eventstore, bufferToJson, logger, rx, invariant, extend, isobjectempty, eventmodels, JSON) {
    return function (_options) {
        ...
        var startDispatching = function startDispatching(_handlers) {
            handlers = _handlers;
            ...
            var relevantEvents = rx.Observable.fromEvent(subscription, 'event').filter(filterEvents).map(createGesEvent, this);
            relevantEvents.forEach(function (vent) {
                return serveEventToHandlers(vent);
            }, function (error) {
                throw error;
            });
        };

        var filterEvents = function filterEvents(payload) {
        ...
        var createGesEvent = function createGesEvent(payload) {
            logger.debug('createGesEvent | event passed through filter');
            var vent = eventmodels.gesEvent(bufferToJson(payload.OriginalEvent.Metadata).eventName, payload.OriginalEvent.Data, payload.OriginalEvent.Metadata, payload.OriginalPosition);
            logger.info('createGesEvent | event transfered into gesEvent: ' + JSON.stringify(vent, null, 4));
            return vent;
        };

        var serveEventToHandlers = function serveEventToHandlers(vent) {
